...
def composite(G, subset_arr, checker, param):
  # Get the values for every level
  MLG_S = []
  l = len(subset_arr)
  ...
  for i in range(l):
    ...
    MLG_S.append(G_S)
  # write the values in test.csv file
  MLGS_costs = ""
  for i in range(l):
  #for k in range(len(MLG_S)):
    k = l-i-1
    G_S = MLG_S[k]
    #print_edges(G_S)
    s = 0
  ...
  f.write(MLGS_costs)
  f.close()
  subprocess.run(["matlab", "-nodisplay", "-nosplash", "-nodesktop", "-r", "run('test_mlgs_composite.m');exit;"])
  f = open('test_out.csv', 'r')
  L_arr = f.readline().split(",")
  L_arr = [int(i) for i in L_arr]
  L_arr = [l-i for i in L_arr]
  L_arr.reverse()

  # this is like topdown
  # We keep to index: i, l_i
  lev_to_i = dict()
  MLG_S = []
  l = len(subset_arr)
  # Traverse the layers top to down
  for i in range(len(L_arr)):
    ...
    # cumpute the abstract graph based on the subset
    G_abs = nx.Graph()
    all_pairs = all_pairs_from_subset(subset_arr[L_arr[i]])
    edges_abs=[]
    for p in all_pairs:
      edges_abs.append((p[0], p[1], nx.shortest_path_length(G, p[0], p[1], 'weight')))
    G_abs.add_weighted_edges_from(edges_abs)
    ...
    for u, v in G_abs.edges():
      ...
      for j in range(1, len(pth)):
        G_S.add_edge(pth[j-1], pth[j], weight=G[pth[j-1]][pth[j]]['weight'])
    G_S = prune(G_S, G, subset_arr[L_arr[i]], checker, param)
    MLG_S.append(G_S)
    lev_to_i[L_arr[i]] = len(MLG_S) - 1

  # Now compute the bottom up for the missing layers
  l = len(subset_arr)
  MLG_S2 = []
  for i in range(l):
    k = l-i-1
    #print(subset_arr[k])
    if k in L_arr:
      G_S = MLG_S[lev_to_i[k]]
    else:
      G_prev = MLG_S2[len(MLG_S2)-1]
      G_S = prune(G_prev, G, subset_arr[k], checker, param)
    MLG_S2.append(G_S)
  return MLG_S2

def bot_up_exact(G, subset_arr, checker, param):
  #G_S = greedy_spanner(G, param)
  G_S = nx.Graph()
  #names, x = single_flow_ilp(G, [subset_arr[len(subset_arr)-1]], checker, param)
  ver_arr = []
  for v in G.nodes():
    ver_arr.append(v)
  names, x = single_flow_ilp(G, [ver_arr], checker, param)
  edges = []
...
def check_bot_up_exact(folder_name, file_name_without_ext, stretch_factor):
  global log_file_name
  ...
  log_file_name = folder_name + '/' + 'print_log_' + file_name_without_ext + '_BU_exact' + '.txt'
  log_file = open(log_file_name, 'w')
  log_file.close()
  filename = folder_name + '/' + file_name_without_ext +'.txt'
  G, subset_arr = build_networkx_graph(filename)
  #subset_arr = [subset_arr[1]]
  MLGS = bot_up_exact(G, subset_arr, multiplicative_check, stretch_factor)
  l = len(MLGS)
  s = 0
  for k in range(len(MLGS)):
    G_S = MLGS[k]
    #print_edges(G_S)
    if not verify_spanner_with_checker(G_S, G, all_pairs_from_subset(subset_arr[l-k-1]), multiplicative_check, stretch_factor):
      print('Wrong!!!')
    #print(G_S.edges())
    for e in G_S.edges():
      s = s + G_S.get_edge_data(e[0], e[1])['weight']
...
def check_bot_up_approx(folder_name, file_name_without_ext, stretch_factor, levels):
  #global log_file_name
  #log_file_name = './print_log_graph_triangle.txt'
  #log_file_name = './print_log_graph_6.txt'
  #log_file_name = './print_log_graph_multi_path.txt'
  ...
  #subset_arr = [subset_arr[1]]
  #MLGS = bot_up_exact(G, subset_arr, multiplicative_check, stretch_factor)
  nodes = int(file_name_without_ext.split('_')[1])
  subset_arr = []
  for l in range(levels):
    ...
    subset.append(steiner_nodes-1)
    subset_arr.append(subset)
  start_bot_up = time.time()
  MLGS = bot_up(G, subset_arr, multiplicative_check, stretch_factor)
  l = len(MLGS)
  s = 0
  for k in range(len(MLGS)):
    G_S = MLGS[k]
    ...
    for e in G_S.edges():
      s = s + G_S.get_edge_data(e[0], e[1])['weight']
      #print(G_S.get_edge_data(e[0], e[1])['weight'])
  print('Total (root+branch&cut) =    ' + str(time.time() - start_bot_up) + ' sec. (16.85 ticks)')
  print('Objective value: ', s)

def check_composite_approx(folder_name, file_name_without_ext, stretch_factor, levels):
  #global log_file_name
  #log_file_name = './print_log_graph_triangle.txt'
  #log_file_name = './print_log_graph_6.txt'
  #log_file_name = './print_log_graph_multi_path.txt'
  ...
  for l in range(levels):
    ...
    subset_arr.append(subset)
  start_bot_up = time.time()
  MLGS = composite(G, subset_arr, multiplicative_check, stretch_factor)
  l = len(MLGS)
  s = 0
  for k in range(len(MLGS)):
    G_S = MLGS[k]
...
def run_bot_up_approx():
  ...
  #folder_name = 'experiment_WS'
  #folder_name = 'experiment_consistent_ER'
  #folder_name = 'experiment_consistent_ER_2'
  #folder_name = 'experiment_consistent_ER_3'
  folder_name = 'experiment_consistent_ER_4'
  f = open(folder_name + '/id_to_file.csv', 'r')
  while True:
   line = f.readline()
   if line=='':
    break
  ...
  # end node
  checker = multiplicative_check
  if write_output:
    output_file = open(folder_name + '/' + 'bottom_up_output.txt', 'w')
    output_file.write('filename;stretch;objective;\n')
    output_file.close()
    time_file = open(folder_name + '/' + 'bot_up_approx_consistent_ER.py', 'w')
    time_file.close()
  for i in range(len(folders)):
  #for i in range(50):
  #for i in range(50, len(folders)):
  #for i in range(53, len(folders)):
    filename = folders[i] + '/' + file_names[i] + '.txt'
    G, subset_arr = build_networkx_graph(filename)
    # subset_arr
    subset_arr = []
    levels = level[i]
    ...
    for k in range(len(subset_arr)):
      ...
      all_pairs = all_pairs_from_subset(subset)
    #print('Time to compute bot up: ', (time.time() - start_bot_up))
    time_file = open(folder_name + '/' + 'bot_up_approx_consistent_ER.py', 'a')
    time_file.write('Time to compute bot up: ' + str(time.time() - start_bot_up) + '\n')
    time_file.close()
    s = 0
    for k in range(len(MLGS)):
      G_S = MLGS[k]
    ...
    #print('Objective value: ', s)
    time_file = open(folder_name + '/' + 'bot_up_approx_consistent_ER.py', 'a')
    time_file.write('Objective value: ' + str(s) + '\n')
    time_file.close()
    if write_output:
      output_file = open(folder_name + '/' + 'bottom_up_output.txt', 'a')
      output_file.write(str(s)+';\n')
...
def single_flow_ilp(G, subset_arr, checker, param):
  ...
  for i in range(len(all_pairs)):
    ...
    #constraint_values.append(param*nx.shortest_path_length(G, u, v, 'weight'))
    constraint_values.append(param*dis[u][v])
    sense = sense + 'L'
    for (i,j,d) in G.edges(data='weight'):
      rows.append(total_rows)
  ...
  for p in range(len(all_pairs)):
    ...
    v = all_pairs[p][1]
    for (i,j,d) in G.edges(data='weight'):
      if dis[u][i] + d + dis[j][v] > param*dis[u][v]:
...
def test_single_flow_ilp(folder_name, file_name_without_ext, stretch_factor):
  ...
  G, subset_arr = build_networkx_graph(filename)
  #subset_arr = [subset_arr[1]]
  single_flow_ilp(G, subset_arr, multiplicative_check, stretch_factor)
...
if __name__ == '__main__':
  ...
  #check_bot_up(multiplicative_check)
  #check_bot_up_exact(sys.argv[1], sys.argv[2], float(sys.argv[3]))
  #run_bot_up_approx()
  #check_bot_up_approx(sys.argv[1], sys.argv[2], float(sys.argv[3]), int(sys.argv[4]))
  check_composite_approx(sys.argv[1], sys.argv[2], float(sys.argv[3]), int(sys.argv[4]))
