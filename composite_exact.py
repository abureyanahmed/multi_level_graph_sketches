...
def top_down_exact(G, subset_arr, checker, param):
  ...
  for i in range(l):
    ...
    #    G_S.add_edge(pth[j-1], pth[j], weight=G[pth[j-1]][pth[j]]['weight'])
    #G_S = prune(G_S, G, subset_arr[i], checker, param)
    names, x = single_flow_ilp(G, [subset_arr[i]], checker, param)
    edges = []
    for j in range(len(names)):
      parts = names[j].split('_')
      if len(parts)==3:
...
def composite_exact(G, subset_arr, checker, param, folder_name, file_name_without_ext):
  ...
  #for k in range(len(MLG_S)):
    ...
    MLGS_costs += str(s)
    if i<l-1:MLGS_costs += ','
  f = open(folder_name + "/" + file_name_without_ext+ '.csv', 'w')
  f.write(MLGS_costs)
  f.close()
  subprocess.run(["matlab", "-nodisplay", "-nosplash", "-nodesktop", "-r", "f='"+ folder_name + "/" + file_name_without_ext+ "';run('test_mlgs_composite.m');exit;"])
  f = open(folder_name + "/" + file_name_without_ext + '_out.csv', 'r')
  ...
  for i in range(len(L_arr)):
    ...
    names, x = single_flow_ilp(G, [subset_arr[L_arr[i]]], checker, param)
    edges = []
    for j in range(len(names)):
      parts = names[j].split('_')
      if len(parts)==3:
...
def check_composite_exact(folder_name, file_name_without_ext, stretch_factor):
  global log_file_name
  #log_file_name = './print_log_graph_triangle.txt'
  #log_file_name = './print_log_graph_6.txt'
  #log_file_name = './print_log_graph_multi_path.txt'
  ...
  #flow_ilp(G, [[0, 1, 2]], multiplicative_check, 2)
  #flow_ilp(G, [[0, 1, 4, 5]], multiplicative_check, 2)
  log_file_name = folder_name + '/' + 'print_log_' + file_name_without_ext + '_CMP_exact' + '.txt'
  log_file = open(log_file_name, 'w')
  log_file.close()
  filename = folder_name + '/' + file_name_without_ext +'.txt'
  G, subset_arr = build_networkx_graph(filename)
  #subset_arr = [subset_arr[1]]
  #MLGS = top_down_exact(G, subset_arr, multiplicative_check, stretch_factor)
  MLGS = composite_exact(G, subset_arr, multiplicative_check, stretch_factor, folder_name, file_name_without_ext)
  s = 0
  for k in range(len(MLGS)):
    G_S = MLGS[k]
    #print_edges(G_S)
    if not verify_spanner_with_checker(G_S, G, all_pairs_from_subset(subset_arr[k]), multiplicative_check, stretch_factor):
      print('Wrong!!!')
    #print(G_S.edges())
    for e in G_S.edges():
      s = s + G_S.get_edge_data(e[0], e[1])['weight']
...
if __name__ == '__main__':
  ...
  #check_top_down_exact(sys.argv[1], sys.argv[2], float(sys.argv[3]))
  check_composite_exact(sys.argv[1], sys.argv[2], float(sys.argv[3]))
